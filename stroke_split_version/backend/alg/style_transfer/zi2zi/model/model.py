...
class Zi2ZiModel:
    ...
    def sample(self, batch, basename):
        ...
        with torch.no_grad():
            ...
            '''
    def sample1(self, batch, basename):
        chk_mkdir(basename)
        cnt = 0
        with torch.no_grad():
            self.set_input(batch[0], batch[2], batch[1])
    ...
    def sample2(self, batch, basename):
        ...
        with torch.no_grad():
            self.set_input(batch[0], batch[2], batch[1])
            self.forward()
            tensor_to_plot = torch.cat([self.real_B,self.fake_B], 3)
            #tensor_to_plot = self.fake_B
            for label, image_tensor in zip(batch[0], tensor_to_plot):
                label_dir = os.path.join(basename, str(label.item()))
                chk_mkdir(label_dir)
                vutils.save_image(image_tensor, os.path.join(label_dir,  "%d_%04d.jpg" % (label, cnt)))
                cnt += 1
            # img = vutils.make_grid(tensor_to_plot)
            # vutils.save_image(tensor_to_plot, basename + "_construct.png")
            '''
